...
public enum Buildings {
    ...
    DRIVETHROUGH("Drive Through", "A take-out service that allows customers to purchase food without leaving their cars.", 880_088D, 264.5D,
                 Material.POWERED_RAIL, 20),
    ...
    RESTAURANT("Restaurant", "A quality business that prepares and serves food and drinks to customers.", 1_000_000_000D, 9_876D, Material.ENCHANTING_TABLE,
               29),
    ...
    CHAIN("Chain", "A set of related restaurants in many different locations that are under shared corporate ownership.", 737_000_000_000D, 773_524D,
          Material.LIGHT_BLUE_GLAZED_TERRACOTTA, 37),
    ...
    HOUSEHOLDNAME("Household Name", "A brand name that is well known to the majority of people.", 150_000_000_000_000D, 123_321_123D, Material.DIAMOND_BLOCK,
                  39);
    ...
    public ItemStack createItem(ClickingPlayer cp) {
        ...
        return new ItemBuilder(getMaterial()).setDisplayName("&3" + getName())
                                             ...
                                             .addLore("&6Amount owned: " + lore3 + cp.getData()
                                                                                     .getSpecificBuildingAmount(this))
                                             .create();
    }

    public double calculateIncome(ClickingPlayer cp) {
        double income = ((getBaseProduction() + cp.getData()
                                                  .getSpecificBuildingsBaseIncreaser(this)) * cp.getData()
                                                                                                .getSpecificBuildingsBaseMultiplier(this) * cp.getData()
                                                                                                                                              .getSpecificBuildingAmount(
                                                                                                                                                      this));
        return income * (1 + FAIRTRADE.getCurrentBonus(cp));
    }

    public double calculateCost(ClickingPlayer cp) {
        double cost = getBaseCost() * cp.getData()
                                        .getSpecificBuildingsCostMultiplier(this) * (Math.pow(1.15, cp.getData()
                                                                                                      .getSpecificBuildingAmount(this)));
